...
function Calendar({
  ...
  return (
    <DayPicker
      ...
      className={cn("p-1.5 md:p-2 text-sm", className)}
      classNames={{
        ...
        table: "w-full border-collapse space-y-1 mt-0",
        head_row: "flex",
        head_cell:
          "text-muted-foreground rounded-md w-7 md:w-8 font-normal text-[0.8rem]",
        row: "flex w-full mt-1",
        cell: "text-center text-sm p-0 relative [&:has([aria-selected])]:bg-accent first:[&:has([aria-selected])]:rounded-l-md last:[&:has([aria-selected])]:rounded-r-md focus-within:relative focus-within:z-20",
        day: cn(
          buttonVariants({ variant: "ghost" }),
          "h-7 w-7 md:h-8 md:w-8 p-0 font-normal aria-selected:opacity-100",
        ),
        day_selected:
